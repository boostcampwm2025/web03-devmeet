...
import { RoomParticipantProps, roomPasswordHashVo, RoomProps, RoomStatusProps, roomStatusVo, roomTitleVo } from "./vo";
...
export class RoomAggregate {
  ...
  private replaceRoom(patch : Partial<RoomProps>) {
    ...
    const nextStatus: RoomStatusProps = patch.status
      ? roomStatusVo(patch.status)
      : current.status;
