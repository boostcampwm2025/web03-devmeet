...
export class CheckUploadFileUsecase<T, ST> {
  ...
  private readonly checkUserAndSelectFileInfoFromCache : CheckUploadFileUsecaseProps<T, ST>["checkUserAndSelectFileInfoFromCache"];
  ...
  private readonly updateFileInfoToCache : CheckUploadFileUsecaseProps<T, ST>["updateFileInfoToCache"];
  ...
  async execute(dto : CheckUploadFileDto) : Promise<CheckUploadFileResult> {
    ...
    const checkResult : CheckUploadFileDtoValidateResult | undefined = await this.checkUserAndSelectFileInfoFromCache.select({ 
      namespace : `${dto.room_id}:${dto.user_id}`, keyName : dto.file_id
    });
    ...
    if ( dto.type === "direct" ) {
      ...
      const checked : boolean = await this.checkUploadFromDisk.check({ pathName, etag : dto.direct.etag });
      ...
      if ( dto.multipart.upload_id !== checkResult.upload_id ) throw new NotAllowRoomFileData("upload_id");

      if ( checkResult.status === "uploading" ) {
        const checked : boolean = await this.checkUploadsFromDisk.checks({ pathName, upload_id : dto.multipart.upload_id, tags : dto.multipart.tags });

        if ( !checked ) throw new NotAllowRoomFileData("multipary tags");

        // 문제없을 경우 결합
        await this.completeUploadToDisk.complete({ pathName, upload_id :dto.multipart.upload_id, size : checkResult.size });
      };
    };
