...
export class DisconnectRoomUsecase<T, CT> {
  ...
  private readonly updateRoomParticipantInfoToDb : DisconnectRoomUsecaseProps<T, CT>["updateRoomParticipantInfoToDb"];
  private readonly deleteRoomDataToCache : DisconnectRoomUsecaseProps<T, CT>["deleteRoomDataToCache"];
  ...
  async execute(dto : DisconnectRoomDto) : Promise<void> {
    ...
    await this.updateRoomParticipantInfoToDb.update({ uniqueValue : dto.room_id, updateColName : "", updateValue : dto.user_id });
