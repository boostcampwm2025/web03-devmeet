...
import {
  ...
  MultipartUploadResumeInfoValue,
  UploadFileDto,
  UploadFileResult,
  UploadUrls,
} from '../dto';
...
export class UploadFileUsecase<T, ST> {
  private readonly checkUserAndSelectPrevFileInfoFromCache: UploadFileUsecaseProps<
    T,
    ST
  ...
  private readonly getCompleteUploadUrlFromDisk: UploadFileUsecaseProps<
    T,
    ST
  >['getCompleteUploadUrlFromDisk'];
  private readonly getMultiVerGroupIdFromDisk: UploadFileUsecaseProps<
    T,
    ST
  ...
  async execute(dto: UploadFileDto): Promise<UploadFileResult> {
    ...
    const prevFileInfo: FindUploadFileInfo | undefined =
      await this.checkUserAndSelectPrevFileInfoFromCache.select({
    ...
    if (dto.size > standardSize && prevFileInfo && prevFileInfo.status === 'uploading') {
      ...
      const status: MultipartUploadResumeInfoValue =
        await this.getCompleteUploadUrlFromDisk.getCompleteMultiId({
          pathName,
      ...
      const missingPartNumbers = this.calcMissingPartNumbers(totalPartCount, uploadedSet);

      // 필요한 urls들만 가져온다.
      const upload_urls: Array<UploadUrls> = await this.getUploadUrlsFromDisk.getUrls({
    ...
    // 이미 같은 파일을 올린적이 있다면 그 파일을 다시 업로드할 필요없이 그대로 주면된다. ( 10mb 이상만 )
      ...
      prevFileInfo &&
      prevFileInfo.status === 'completed' &&
      prevFileInfo.upload_id &&
      dto.size > standardSize
    ) {
      result = {
        file_id: prevFileInfo.file_id,
    ...
    if (dto.size <= standardSize) {
      const upload_url: string = await this.getUploadUrlFromDisk.getUrl({
        pathName,
        mime_type: dto.mime_type,
      });
    ...
    // 10mb 초과
      ...
      // 새로운 upload_id 발급
      const upload_id: string = await this.getMultiVerGroupIdFromDisk.getMultiId({
        pathName,
        mime_type: dto.mime_type,
      });

      const totalPartCount = this.calcPartCount(dto.size, standardSize);
      const partNumbers = Array.from({ length: totalPartCount }, (_, i) => i + 1);
      const upload_urls: Array<UploadUrls> = await this.getUploadUrlsFromDisk.getUrls({
        upload_id,
        pathName: pathName.join('/'),
        partNumbers,
    ...
    const insertDto: InsertUploadFileInfoDto = {
      ...dto,
      upload_id: result.multipart?.upload_id,
  ...
  // 현재 없는 url만 가져온다.
  private calcMissingPartNumbers(totalPartCount: number, uploaded: Set<number>): number[] {
