...
export class DownLoadFileUsecase<T, ST> {
  private readonly checkRoomMemberFromCache: DownLaodFileUsecaseProps<
    T,
    ST
  >['checkRoomMemberFromCache'];
  private readonly getUploadUrlFromDisk: DownLaodFileUsecaseProps<T, ST>['getUploadUrlFromDisk'];

  constructor({ checkRoomMemberFromCache, getUploadUrlFromDisk }: DownLaodFileUsecaseProps<T, ST>) {
  ...
  async execute(dto: DownLoadFileDto): Promise<string> {
    ...
    const fileName: string | undefined = await this.checkRoomMemberFromCache.select({
