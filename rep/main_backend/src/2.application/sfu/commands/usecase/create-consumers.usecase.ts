...
import { CreateConsumerResults, CreateConsumersDto, CreateConsumersProducerInfoDto, InsertConsumerDatasDto } from "../dto";
...
export class CreateConsumersUsecase<T> {
  ...
  async execute(dto : CreateConsumersDto) : Promise<CreateConsumerResults> {
    ...
    if ( dto.room_id !== transportInfo.room_id || dto.socket_id !== transportInfo.socket_id || transportInfo.type !== "recv" || dto.user_id !== transportInfo.user_id ) throw new SfuErrorMessage("consumer를 위한 transport_id가 올바르지 않습니다 다시 확인해주시길 바랍니다.");

    // transport 찾기
    const transport = this.transportRepo.get(dto.transport_id);
    if ( !transport ) throw new SfuErrorMessage("consumer를 위한 transport_id가 올바르지 않습니다 다시 확인해주시길 바랍니다.");
