...
export class CreateProduceUsecase<T> {
  ...
  async execute(dto : CreatePropduceDto) : Promise<CreateProduceResult> {
    ...
    const transportInfo : RoomTransportInfo = await this.selectTransportDataFromCache.select({ namespace : dto.transport_id, keyName : "" });
    if ( dto.room_id !== transportInfo.room_id || dto.socket_id !== transportInfo.socket_id || transportInfo.type !== "send" || dto.user_id !== transportInfo.user_id ) throw new SfuErrorMessage("produce를 위한 transport_id가 올바르지 않습니다 다시 확인해주시길 바랍니다.");
    ...
    if ( dto.type === "cam" || dto.type === "mic" ) {    
      ...
      const userChecked : boolean = await this.selectUserProducerDataFromCache.select({ namespace : `${dto.room_id.trim()}:${dto.user_id.trim()}`.trim(), keyName : dto.kind });
      if ( userChecked ) throw new SfuErrorMessage(`${dto.type}은 이미 실행되어 있습니다.`);
    } else {
      // -> main에 현재 데이터가 존재하는지 확인
      const mainChecked : boolean = await this.selectMainProducerDataFromCache.select({ namespace : dto.room_id, keyName : dto.type });
