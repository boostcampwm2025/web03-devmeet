...
export class ResumeConsumersUsecase<T> {
  ...
  async execute(dto : ResumeConsumersDto) : Promise<void> {
    ...
    const checkConsumerIds : Array<string> = await this.selectConsumerInfosFromCache.selectKeys({ namespace : `${dto.room_id}:${dto.user_id}`, keyNames : dto.consumer_ids });
