...
import { CompletedPart, CompleteMultipartUploadCommand, ListPartsCommand, ListPartsCommandOutput, Part, type S3Client } from "@aws-sdk/client-s3";
...
export class CompleteUploadToAwsS3 extends CompleteUploadFileToDisk<S3Client> {
  ...
  async complete({ pathName, upload_id, size }: { pathName: string; upload_id: string; size: number; }): Promise<void> | never {
