    ...
    ClientsModule.registerAsync([
      {
        ...
        useFactory : (config : ConfigService) => {
          ...
          const brokers : Array<string> = config.get<string>('NODE_APP_KAFKA_BROKERS', "127.0.0.1:9092")
          .split(',')
          .map(v => v.trim());
          const groupId = config.get<string>('NODE_APP_KAFKA_GROUP_ID', "groupid");
        
          // sasl 
          const saslMechanism = config.get<string>('NODE_APP_KAFKA_SASL_MECHANISM', "scram-sha-256") as "scram-sha-256"; // 가장 보안적으로 않좋다. 
          // ssl에 쓰이는 이름
          ...
          return {
            ...
            options: {
              ...
              consumer: { groupId },
              subscribe: { fromBeginning: config.get<string>("NODE_ENV", "deployment") === "deployment" },
            },
          }
        }
      }
